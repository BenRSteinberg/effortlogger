...
function query() {
    database.performQuery({ recordType: 'effortloggerdata' }).then(function (response) {
        eventcount++;
        if (response.isServerError == true) {
            ...
            while (response.records.length > a) {
                ...
                outputText += `${response.records[a].fields.Category.value}\n`;
                outputText += `${response.records[a].fields.date.value}\n`;
                outputText += `${response.records[a].fields.deliverable.value}\n`;
                outputText += `${response.records[a].fields.dtime.value}\n`;
                outputText += `${response.records[a].fields.lifecycle.value}\n`;
                outputText += `${response.records[a].fields.start.value}\n`;
                outputText += `${response.records[a].fields.stop.value}\n`;
                outputText += `${response.records[a].fields.user.value}\n`;
                outputText += `${response.records[a].recordName}\n`;
                a++;
            }
            //console.log(outputText);
            // Write the output text to the file
            fs.writeFileSync('consoleOutput.txt', outputText);
